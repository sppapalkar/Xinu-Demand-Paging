...
SYSCALL write_dirty(int pid){
	STATWORD ps;
	pd_t *pd;
	pt_t *pt;
	int i,j, index, store, pageth;
	disable(ps);
	...
	for(i = 0; i<4; i++){
		if((pd+i)->pd_pres == 1){
			pt = (pt_t*) ((pd+i)->pd_base*NBPG);
			for(j = 0; j<1024; j++){
				if((pt+j)->pt_base >= FRAME0 && (pt+j)->pt_pres == 1 && (pt+j)->pt_dirty == 1){
					index = (pt+j)->pt_base - FRAME0;
					if(frm_tab[index].fr_pid == pid && frm_tab[index].fr_vpno != -1){
						store = -1;
						pageth = -1;
						if(bsm_lookup(pid, frm_tab[index].fr_vpno*NBPG, &store, &pageth) != SYSERR){
							// kprintf("\nFrame: %d, Pid: %d, VPNO: %d, Store: %d, Page: %d", index, frm_tab[index].fr_pid, frm_tab[index].fr_vpno, store, pageth);
							write_bs(((index+FRAME0)*NBPG), store, pageth);
							invalidate_pte(frm_tab[index].fr_pid, frm_tab[index].fr_vpno);
							update_ipt(index, 0, 0, FRM_UNMAPPED);
						}
					}	
				}
			}
...
// 	int i, j;
// 	kprintf("\nPid: %d; Dirty Pages: ",currpid);
// 	for(i = 0; i<4; i++){
// 		if((pd+i)->pd_pres == 1){
// 			pt_t *pt = (pd+i)->pd_base*NBPG;
// 			for(j = 0; j<1024; j++){
// 				if((pt+j)->pt_pres == 1 && (pt+j)->pt_dirty == 1){
// 					if((pt+j)->pt_base > 1023){
// 						kprintf("\nPID: %d, Page frame no: %d", frm_tab[(pt+j)->pt_base - 1024].fr_pid, (pt+j)->pt_base);
// 						kprintf("\nVPNO: %d", frm_tab[(pt+j)->pt_base - 1024].fr_vpno);
...
int	resched()
{
	...
	PrintSaved(nptr);
#endif
	if(oldpid != currpid && debug_mode != 1){
		write_dirty(oldpid);
	}
	write_cr3(nptr->pdbr);
